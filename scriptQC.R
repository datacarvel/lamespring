# ...
# QUÉBEC : Une station a temps + nuages/weather 51457 mais pas précips, celle précips 26892 n'a pas nuages/weather

qc7016293 <- weather_dl(station_ids = "51457", start = "2018-04-01", end = "2025-05-31", interval = "day") # temps nuages QC INTL A
qc26892 <- weather_dl(station_ids = "26892", start = "2018-04-01", end = "2025-05-31", interval = "day") # precips QUEBEC/JEAN LESAGE
...
qchour <- weather_dl(station_ids = "51457", start = "2019-03-31", end = "2025-05-31", interval = "hour")
qchour2 <- qchour
...
qchour2$filled_weather <- fill_weather_gaps(qchour2$weather)

qchour_light <- qchour2 %>%
  select(
...
qcClouds <- ggcal(
  fills = qchour_calc$sumClouds, 
  dates = qchour_calc$date,
  days = qchour_calc$day,
  calc = "sum",
  unitm = "h"
) +
  scale_fill_gradient(
...
#RainSum <- qc %>% group_by(year, month) %>% reframe(sum = sum(total_precip, na.rm = T))

qcRain <- ggcal(
  fills = qcpluie$total_precip, 
  dates = qcpluie$date,
  days = qcpluie$day,
  calc = "sum",
  unitm = "mm"
...
THEQC <- (qcTemps / qcClouds / qcRain) + 
  plot_annotation(
    title = "Températures, temps nuageux et pluie en avril et mai à Québec",
...
ggsave("QC.png", plot = THEQC, width = (1912 / 144), height = (970 / 144), dpi = 200, bg = "#FEFCE9")